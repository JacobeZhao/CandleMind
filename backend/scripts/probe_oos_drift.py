...
def load_bundle(symbol, target):
    model_path = MARKET_ROOT / 'models' / f'{symbol}_{target}.pkl'
...
for sym, entry_thr in [('BTCUSDT', 0.62), ('BNBUSDT', 0.58)]:
    ...
    feat_path  = MARKET_ROOT / 'features_ml' / f'{sym}_features.parquet'
    ...
    for target, col, thr in [('long_label', 'long_prob', entry_thr),
        ...
        all_feat_cols = pq.read_schema(str(feat_path)).names
        ...
        if k_train in results and k_oos in results:
            val_auc = results.get(k_val, {}).get('auc', 'N/A')
            print(f"\n    >>> AUC漂移:      {results[k_train]['auc']:.4f}(训练) → {val_auc} (验证) → {results[k_oos]['auc']:.4f}(OOS)")
            print(f"    >>> mean_prob漂移: {results[k_train]['mean']:.4f}(训练) → {results[k_oos]['mean']:.4f}(OOS)"
                  f"  Δ={results[k_oos]['mean']-results[k_train]['mean']:+.4f}")
            print(f"    >>> 超阈值频率:    {results[k_train]['above_thr']:.3f}(训练) → {results[k_oos]['above_thr']:.3f}(OOS)"
                  f"  Δ={results[k_oos]['above_thr']-results[k_train]['above_thr']:+.3f}")
    ...
    if sym == 'BNBUSDT':
        ...
        try:
            ...
            need_cols = list(dict.fromkeys(
                ['open_time'] + bundle_s.feature_cols + bundle_l.feature_cols
            ))
            import pyarrow.parquet as pq
            all_feat_cols_bnb = pq.read_schema(str(feat_path)).names
            ...
            print(f"\n  月度统计 (BNB OOS):")
            hdr = (f"  {'月份':<10} {'close均价':>10} {'short_mean':>10} {'short>0.58':>10}"
                   f" {'long_mean':>10} {'long>0.58':>10} {'sl_rate':>8} {'ll_rate':>8}")
            print(hdr)
            print(f"  {'-'*85}")
            ...
            for month, g in oos_bnb.groupby('month'):
                ...
                print(f"  {str(month):<10} {close_info:>10} {s_mean:>10.4f} {s_above:>10.3f}"
                      f" {l_mean:>10.4f} {l_above:>10.3f} {sl_rate:>8.4f} {ll_rate:>8.4f}  n={len(g):,}")
