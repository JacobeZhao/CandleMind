...
CandleMind は FastAPI と React で構築され、リアルタイム市場データ、トレンド分析、戦略設定、取引所での注文執行、口座統計を一つのワークスペースにまとめます。**CandleMind トレンド戦略**を中心に、常時稼働する AI 市場アシスタントと再現可能なオフライン研究基盤を提供します。

## 主な機能

| 機能 | 説明 |
| --- | --- |
| リアルタイム市場 | Binance WebSocket、ローソク足、メインチャート指標、主要市場指標 |
...
3. クラウド AI の Base URL は信頼済み HTTPS ホストに限定されます。ローカル Provider はループバックまたは RFC1918 アドレスを利用できます。詳細は [`docs/AI_CONFIGURATION.md`](docs/AI_CONFIGURATION.md) を参照してください。
4. メインネットには testnet 検証、サーバー側スイッチ、画面上の明示確認がすべて必要です。
5. 実資金を使用する前に、戦略、ポジション量、レバレッジ、損切り、取引所権限を独自に確認してください。
