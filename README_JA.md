<div align="center">
  ...
  <h1>CandleMind</h1>
  <p><strong>複数市場に対応するオープンソースのトレンド取引研究・自動執行プラットフォーム</strong></p>
  <p>
...
CandleMind は暗号資産取引所や A 株などの複数市場を対象とし、統一された市場セレクターと取引所アダプターによって市場データ、口座、注文執行を分離します。現在稼働しているのは **Binance Futures** です。OKX、Bybit、Gate.io、A 株は製品画面に用意されていますが、まだ接続されていません。未稼働の市場を選択すると未接続状態が表示され、Binance API へ誤ってフォールバックすることはありません。

## 主な機能

| 機能 | 説明 |
| --- | --- |
| リアルタイム市場 | 統一された複数市場ワークスペース。現在は Binance WebSocket、ローソク足、メインチャート指標を提供 |
...
2. 現在稼働中の Binance 連携と AI Provider の認証情報は設定画面から登録し、`trader.db` と `secret.key` を一緒にバックアップしてください。
